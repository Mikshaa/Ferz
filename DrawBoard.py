...
class Board:
    def __init__(self, speed=1000, size=50, N=8):
        self.speed = speed
        t.speed(speed)
        self.SIZE = size
        self.N = N
    ...
    def ferz(self, j, k):
        ...
        if (j+k)%2==1: #if для выбора цвета ферзя потому что клетки могут быть белого и черного цвета
            t.pencolor('white') #функция смены цвета
        self.rectangle(x,y,self.SIZE, True) #вызов функций далее опишу что они значут
    ...
    def rectangle(self, x,y, size,check): #функция рисования прямоугольника внутри ферзя
        ...
        t.pendown()
        for i in range(2):
            t.fd(size*0.6) #fd это forward, так можно сокращать
            t.lt(90) #left
            t.fd(size/8)
            t.lt(90)
